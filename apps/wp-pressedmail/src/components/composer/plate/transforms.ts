...
/**
 * Block insert / turn-into transforms (template transforms.ts port).
 *
 * Powers the Insert dropdown, Turn-into dropdown, slash menu and block
 * context menu. Compared to the playground template this drops the
 * code-drawing, footnote, math and drawing entries (optional feature stacks
 * not shipped in the default PressedMail composer bundle).
 */

import { insertCallout } from '@kit/plate/callout';
import { insertCodeBlock, toggleCodeBlock } from '@kit/plate/code-block';
import { insertDate } from '@kit/plate/date';
import { insertColumnGroup, toggleColumnGroup } from '@kit/plate/layout';
import { triggerFloatingLink } from '@kit/plate/link/react';
import {
...
import { insertToc } from '@kit/plate/toc';
import {
  KEYS,
  type NodeEntry,
  type Path,
...
> = {
  ...
  [KEYS.table]: (editor) =>
    editor
      ...
      .insert.table({ colCount: 2, rowCount: 2 }, { select: true }),
