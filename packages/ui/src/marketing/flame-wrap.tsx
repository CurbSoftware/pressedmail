...
export type FlameColor = [number, number, number] | string;

type FlameRgb = [number, number, number];
...
/** Resolve a semantic CSS color to the normalized sRGB tuple WebGL expects. */
function resolveFlameColor(
  color: FlameColor,
  referenceElement: HTMLElement,
): FlameRgb {
  if (Array.isArray(color)) {
    return color.map(clampColorChannel) as FlameRgb;
  }
  ...
  try {
    ...
    if (!context || !resolvedColor) return DEFAULT_COLOR;
...
export function createFlameWrap(
  ...
  // Keep the wrapped UI in ordinary DOM. Experimental HTML-in-canvas capture
  // can report support while producing an empty texture, which hides the real
  // card and leaves only the flame visible.
  ...
  if (htmlInCanvas) {
    paintable.onpaint = () => {
      try {
        ...
        // Experimental HTML-in-canvas paints can fail without invalidating
        // the normal DOM fallback or the WebGL outline.
      }
    };
  }
  ...
  function requiredUniform(name: string) {
    const location = gl!.getUniformLocation(program, name);

    if (location === null) {
      throw new Error(`FlameWrap shader uniform is unavailable: ${name}`);
    }
  ...
  const uniforms = {
    uContent: requiredUniform('uContent'),
    uResolution: requiredUniform('uResolution'),
    uTime: requiredUniform('uTime'),
    uRectCenter: requiredUniform('uRectCenter'),
    ...
    uScale: requiredUniform('uScale'),
    uTurbulence: requiredUniform('uTurbulence'),
    uTurbScale: requiredUniform('uTurbScale'),
    uTurbReach: requiredUniform('uTurbReach'),
    uSparks: requiredUniform('uSparks'),
    uSparkSize: requiredUniform('uSparkSize'),
    uSparkDensity: requiredUniform('uSparkDensity'),
    uSparkSpeed: requiredUniform('uSparkSpeed'),
    uRim: requiredUniform('uRim'),
    uMelt: requiredUniform('uMelt'),
    uDistortion: requiredUniform('uDistortion'),
    uSmoke: requiredUniform('uSmoke'),
    uEmber: requiredUniform('uEmber'),
    uScorch: requiredUniform('uScorch'),
    uHasContent: requiredUniform('uHasContent'),
  };
  ...
  function syncCanvasSize() {
    ...
    const scaleX = outRect.width / output.clientWidth;
    const scaleY = outRect.height / output.clientHeight;
    if (outRect.width > 0 && boxRect.width > 0 && scaleX > 0 && scaleY > 0) {
      rect.cx = ((boxRect.left + boxRect.right) / 2 - outRect.left) / scaleX;
      rect.cy = (outRect.bottom - (boxRect.top + boxRect.bottom) / 2) / scaleY;
      rect.hx = boxRect.width / scaleX / 2;
      rect.hy = boxRect.height / scaleY / 2;
    }
    if (htmlInCanvas) {
  ...
  return {
    setOptions(next) {
      ...
      Object.assign(config, resolvedNext);
      if (!changed) return;
      syncCanvasSize();
      start();
    },
...
export function FlameWrap({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('FlameWrap could not initialize:', error);
    }
