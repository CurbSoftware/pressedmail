...
function TabsContent({ className, ...props }: TabsPrimitive.Panel.Props) {
  return (
    <TabsPrimitive.Panel
      ...
      // base-ui makes the panel a tab stop, so `outline-none` on its own left a
      // focusable element with nothing to show for it. Keep the resting state
      // clean and paint a ring only when focus arrives from the keyboard,
      // matching TabsTrigger above.
      className={cn(
        'focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:outline-ring flex-1 rounded-md text-sm outline-none focus-visible:ring-[3px] focus-visible:outline-1',
        className,
      )}
      {...props}
    />
