...
import { NavigationConfigSchema } from '../makerkit/navigation-config.schema';
...
function Sidebar({
  ...
  side?: 'left' | 'right';
  variant?: 'ghost' | 'sidebar' | 'floating' | 'inset';
  collapsible?: 'offExamples' | 'icon' | 'none';
}) {
  const { isMobile, state, openMobile, setOpenMobile } = useSidebar();
  const resolvedVariant = variant === 'ghost' ? 'sidebar' : variant;
...
function MaybeCollapsible({
  ...
  // Manage local state internally to avoid warnings when defaultOpen changes
  const [open, setOpen] = React.useState(defaultOpen ?? false);

  // Update local state when defaultOpen changes
  React.useEffect(() => {
