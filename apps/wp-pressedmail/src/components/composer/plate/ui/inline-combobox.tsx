...
import { filterWords } from "@kit/plate/combobox";
import {
  type UseComboboxInputResult,
  useComboboxInput,
  useHTMLInputCursorState,
} from "@kit/plate/combobox/react";
import { cva } from "class-variance-authority";
import type { PointRef, TElement } from "@kit/plate";
import { useComposedRef, useEditorRef } from "@kit/plate/react";
...
const InlineComboboxContent: typeof ComboboxPopover = ({
  ...
  return (
    <Portal>
      {/* The popover portals into document.body, outside the plugin's
          #pressedmail-plugin scope, so the themed-scrollbar CSS
          (tailwind-base.css, scoped to [data-pm-portal]) would not reach it and
          the slash menu showed a default browser scrollbar.
          - data-pm-portal on the scroller itself gives it the standard
            `scrollbar-width: thin` + `scrollbar-color: var(--border) transparent`
            (scrollbar-width does NOT inherit, so it must be on the element),
            thin themed scrollbar in Firefox and Chrome.
          - data-pm-portal on the wrapper makes the scroller a descendant of the
            scope, so the webkit rule `[data-pm-portal] *::-webkit-scrollbar`
            also themes it (8px, var(--border) thumb, rounded) in Chrome.
          Result: the slash menu scrollbar matches the rest of the app. */}
      <div data-pm-portal lang={toBcp47(locale)}>
        <ComboboxPopover
          data-pm-portal
          className={cn(
...
function InlineComboboxGroup({
  ...
  // Render plainly: this app's Tailwind build does not emit the
  // `[&:has([role=option])]` arbitrary variant, so the upstream
  // `hidden ... [&:has([role=option])]:block` pattern left every group
  // display:none (empty slash/mention popover). Groups here always contain
  // items, so they render as block with a separator between siblings.
  return (
    <ComboboxGroup
      {...props}
      className={cn("block py-1.5 not-last:border-b", className)}
    />
  );
}
