...
/* template pattern: @platejs/emoji hands
   out a refs container that must be attached during render. */

/**
 * Emoji picker toolbar button (template emoji-toolbar-button.tsx port).
 * Full emoji-mart picker via @platejs/emoji (search, category navigation,
 * preview), replaces the old hardcoded emoji grid. Differences from the
 * template: the popover uses the plugin Popover (theme-scoped portal), and
 * hover states use plain `hover:` classes instead of `group-hover:` overlay
 * divs (this app's Tailwind build does not emit some arbitrary variants).
 */
...
import {
  ...
  EmojiSettings,
  type GridRow,
} from '@kit/plate/emoji';
import {
  type EmojiDropdownMenuOptions,
  type UseEmojiPickerType,
  useEmojiDropdownMenuState,
} from '@kit/plate/emoji/react';
import {
  AppleIcon,
  ClockIcon,
  CompassIcon,
...
const EmojiButton = React.memo(function EmojiButton({
  ...
  return (
    <button
      ...
      className="relative flex size-9 cursor-pointer items-center justify-center rounded-full border-none bg-transparent text-2xl leading-none hover:bg-muted"
      data-index={index}
      ...
      emojiLibrary
        ...
        .map(({ id: categoryId }) => {
          ...
          return (
            ...
            >
              <div className="sticky -top-px z-10 bg-popover/90 p-1 py-2 text-sm font-semibold backdrop-blur-sm">
  ...
  const SearchList = React.useCallback(
    () => (
      <div data-id="search" style={{ width: getRowWidth }}>
        <div className="sticky -top-px z-10 bg-popover/90 p-1 py-2 text-sm font-semibold text-card-foreground backdrop-blur-sm">
          {i18n.searchResult}
        </div>
...
function EmojiPickerSearchAndClear({
  ...
  return (
    <div className="flex items-center text-foreground">
      <div className="absolute left-2.5 top-1/2 z-10 flex size-5 -translate-y-1/2 items-center justify-center text-foreground">
        {emojiSearchIcons.loupe}
      </div>
      {searchValue && (
        <Button
          aria-label={__('Clear', 'pressedmail')}
          className="absolute right-0.5 top-1/2 flex size-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border-none bg-transparent text-popover-foreground hover:bg-transparent"
          onClick={clearSearch}
          size="icon"
          title={i18n.clear}
          type="button"
