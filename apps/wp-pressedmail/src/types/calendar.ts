 ...
 * expected.
 *
 * These live here, not beside the occurrence helpers, because the Free build's
 * inert context stub is typed with them. A type import is erased by Rollup but
 * still names its module, so declaring the context value over there published
 * the Pro calendar implementation as Free source.
 */
