...
const LEGACY_PLUGIN_REST_NAMESPACE = "api/v1";

function normalizeRestNamespace(namespace: unknown): string {
  if (typeof namespace !== "string") {
    return LEGACY_PLUGIN_REST_NAMESPACE;
  }

  const normalized = namespace.trim().replace(/^\/+|\/+$/g, "");
  return normalized || LEGACY_PLUGIN_REST_NAMESPACE;
}

/**
 * Runtime plugin REST namespace. Fresh builds localize `pressedmail/v1`; older
 * payloads fall back to the legacy `api/v1` namespace.
 */
